...
leaf <- read.csv("~/Google Drive/Rudolf_MRE/results/ch2/metabolism/fba/media/csources2.csv", header = T, sep = ",") %>% 
...
#     FBA models
setwd("~/Google Drive/Rudolf_MRE/results/ch2/metabolism/fba/sbml/short")
...
biomass$name = str_replace_all(as.character(biomass$name), 
                                 setNames(as.character(label$nameB), as.character(label$name)))
...
# Save files!
write.csv(biomass,"~/Google Drive/Rudolf_MRE/results/ch2/metabolism/final/biomass_fba_short.csv")
write.csv(melt(as.matrix(matd), varnames = c("sp1", "sp2"), value.name = "fba"),
          "~/Google Drive/Rudolf_MRE/results/ch2/metabolism/final/dist_fba_short.csv")
write.csv(melt(mat, varnames = c("resource", "sp1"), value.name = "pred.cprof"),
          "~/Google Drive/Rudolf_MRE/results/ch2/metabolism/final/dist_fba2_short.csv")
pdf("~/Google Drive/Rudolf_MRE/results/ch2/metabolism/final/heatmap_fba_auc.pdf", height = 5, width = 10, useDingbats = F)
draw(hm %v% h1)
dev.off()
